...
class derivative():
    ...
    def price(self,vol=0,N=500,n=100):
        ...
        start=(td.year,td.month,td.day),end=self.T,vol=vol,N=N)
        ...
        if self.type_ == 'european':
            ...
            if self.check_BS==1:
                BS_value = BS(self.s[0],self.K,self.r,self.std,self.T,option_type=self.option_type)
                error = v - BS_value
...
class call(derivative):
    def f(self,S):
        ...
        self.check_BS = 1
        #print('Type of derivative: {}'.format(self.option_type))
...
def BS(S,K,r,std,t,option_type='call'):
